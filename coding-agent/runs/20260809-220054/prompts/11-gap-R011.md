...
```
# Requirement: Predator AI: hunting the player  (GDD 2.2)

**What the design requires**
Creatures of the player's size or larger hunt the player with the same lock/pounce rules: a pulsing orange halo warns the player of a lock, smaller attackers stand still to wind up while bigger ones keep closing, the pounce direction commits only at the end of the wind-up, and running far enough breaks the lock.

**How you could tell it works in-game**
When an enemy locks the player an orange halo pulses; sidestepping during a small enemy's stationary wind-up makes its pounce miss, hitting it during the wind-up cancels it, and sprinting away drops the halo. Larger enemies close distance while the halo is up.

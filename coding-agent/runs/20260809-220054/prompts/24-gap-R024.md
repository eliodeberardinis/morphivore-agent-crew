...
```
# Requirement: Meat tiers and their world distribution  (GDD 2.4b, 2.7)

**What the design requires**
Each coloured creature is a tier of a colour and its meat carries that tier into the player's limb. Pale wanders in every biome; Dusk debuts in Prairies pockets and wanders from the Wetlands on; Deep debuts in Wetlands pockets and wanders from the Mountains on; Rage debuts in Mountains pockets and wanders the deep ends of Beach and Volcanic; Clash never wanders and appears only on elites, trait minibosses and Alphas.

**How you could tell it works in-game**
In the Prairies, Dusk meat is only found inside a defended pocket while Pale roams the open; by the Wetlands Dusk wanders freely; two-toned Clash creatures are only ever elites, minibosses or Alphas.

**Data it implies**
creatures.json: meat tier, colour family, role, rank, home biomes; biomes.json: which tiers wander vs pocket-only per biome

**Worked example from the document**
Tier debut/roam schedule: Dusk pockets in Prairies → roams Wetlands+; Deep pockets in Wetlands → roams Mountains+; Rage pockets in Mountains → roams deep Beach and Volcanic.
...
exactly one form** — no recipes, no invalid states: - **Family** = the colour holding the most limbs; ties break toward the **most recently eaten** (at birth, a fixed precedence Yellow → Red → Blue → Purple → Grey, so every litter-mate resolves to a definite form). - **Intensity** = the **lowest tier among your family-colour units, stepped down one rung for every limb *not* carrying the family** (the "lowest-link" rule), floored at Pale. - **The dilution ladder is the four wild tiers** (Pale, Dusk, Deep, Rage); **Clash sits beside Rage,** a parallel summit reached only by holding Clash units; diluting either slides it to **Deep.** Holding any Clash unit wins the tie for Clash. - **All limbs white** = the blank (form zero). You mutate the instant the resolution changes. **Your weakest meat sets your intensity,** so climbing to Rage means pooping out every weak unit for a strong one — full-body commitment, meal by meal. **Why six limbs:** the body is a **cube with six faces to grow from,** so a form spans up to six limb-segments in an authored body plan — **the socket layout is the authored silhouette,** and 150 distinguishable bodies come from one procedural system (silhouette reads family, segment count rank, saturation tier). Traits render on the cube's own face (§2.6a). The per-run roll (§2.7) can spawn an **over-tier pocket** (a Rage hollow in the Prairies), so every rung is reachable across enough runs. **Intensity is orthogonal to rank** — a 2-limb Rage Brawler and a 5-limb Pale Brawler are both Yellow but play

...
```
# Requirement: Combat pacing invariant (2–3 pounces, no one-shots)  (GDD 2.5)

**What the design requires**
Stat tuning must keep a knockdown at 2–3 pounces against appropriate targets and more when mismatched, and must never allow a one-shot: the player's Damage is lower than every prey Health bar at every rank.

**How you could tell it works in-game**
Appropriate prey always requires at least two pounces to knock down; no single pounce ever knocks a prey creature straight into the dizzy state at any rank.

**Data it implies**
(none)

**Worked example from the document**
For every rank, player Damage < every prey Health bar; knockdown of an appropriate target takes 2–3 pounces.

...
```
# Requirement: Lock-on targeting  (GDD 2.1)

**What the design requires**
Holding right-click / left trigger locks the nearest creature inside the player's facing cone; lock range is a stat that grows as the player evolves, and the lock breaks if the target gets far enough away.

**How you could tell it works in-game**
A red ring snaps over the nearest creature ahead while the button is held; releasing or running away removes the ring. A higher-rank/longer-reach form locks targets that a one-limb blank could not.

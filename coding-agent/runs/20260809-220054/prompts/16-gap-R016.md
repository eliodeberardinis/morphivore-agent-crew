...
```
# Requirement: Final stat composition (rank baseline × family × intensity)  (GDD 2.4)

**What the design requires**
A form's live stats are the limb baseline multiplied by the family multiplier scaled by intensity, producing the player's current Health max, Damage, Speed, Reach and Dash.

**How you could tell it works in-game**
The HUD bar length and observable damage/speed change immediately and consistently when either rank or form changes; no colour is strictly best because Health is one of the multiplied axes.

**Data it implies**
forms.json stat profile per form

**Worked example from the document**
A 2-limb Rage Brawler and a 5-limb Pale Brawler are both Yellow and must produce clearly different stat lines from the same tables.

...
```
# Requirement: Single Health meter and permadeath  (GDD 1, 2.3)

**What the design requires**
The player has exactly one meter, Health; when it reaches zero the bloodline ends and the run is over permanently.

**How you could tell it works in-game**
One bar is shown on the HUD and no second resource exists; taking lethal damage ends the run outright with no continue.

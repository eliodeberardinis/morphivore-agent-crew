...
```
# Requirement: Breeding, litters and rank gain  (GDD 2.5)

**What the design requires**
With an emblem claimed the player breeds: a litter is offered, each offspring one limb larger, and the player continues as the chosen one. Litter size = 2 + (distinct forms this biome − the biome's form gate), capped at 5. Offspring inherit rank, all traits, emblems and panels; what varies is which colours their limbs are born loaded with, always at Pale — committed (monochrome) or flexible (mixed). Limbs cap at six, and rank is gained only this way, never by eating.

**How you could tell it works in-game**
After eating an emblem the player picks from 2–5 offspring and resumes play one limb larger with the same traits/emblems/panels but a new starting buffer of Pale colours; eating hundreds of creatures never adds a limb; a six-limb beast is offered no further breeding.

**Data it implies**
Run state: rank, offspring definitions (starting buffer per litter-mate)

**Worked example from the document**
A bare clear of the form gate gives two offspring; three distinct forms past the gate caps the litter at five.
...
(monochrome, an intensity in hand) or **flexible** (mixed). **The ones you didn't pick** appear in the Bestiary as known targets. Limbs cap at **six.** Each new limb opens a **new** 25-form column; because the gate climbs (3→5→7→9→11) against a flat 25-form pool, litter-width headroom tightens with rank.

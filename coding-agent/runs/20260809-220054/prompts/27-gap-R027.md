...
```
# Requirement: Pockets, distance-based difficulty and lair placement  (GDD 2.7, 2.4b)

**What the design requires**
Ordinary creatures wander everywhere so a meal is always findable, but the population mix darkens with distance from the player's entrance; top-tier meat clusters in defended pockets that bite back with swarming packs and elites; the Alpha lair sits at the strong end; and the per-run roll can place an over-tier pocket (e.g. a Rage hollow in the Prairies).

**How you could tell it works in-game**
Walking away from the entrance meets progressively stronger creatures; the strongest meat is only inside pockets guarded by packs and elites; some runs contain a pocket holding meat above the biome's normal ceiling.

**Data it implies**
biomes.json: pocket count, palette weights, population/respawn

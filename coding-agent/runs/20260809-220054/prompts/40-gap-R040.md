...
```
# Requirement: Bestiary persistence across runs  (GDD 1, 2.9)

**What the design requires**
The Bestiary is the only persistence: every distinct form any bloodline has ever taken and every emblem claimed is saved across runs and survives death, while all in-run progress (traits, panels, emblems as keys, rank) is lost when Health hits zero.

**How you could tell it works in-game**
Dying returns the player to a one-limb blank cube with no traits or panels, yet the Bestiary still shows every form and emblem discovered before the death, including in the run just lost.

**Data it implies**
Save file holding discovered form ids and claimed emblems (no run state)

...
```
# Requirement: Trait permanence, face rendering and inheritance  (GDD 2.6a)

**What the design requires**
Traits are body, not limbs: they render on the main cubic face, and once acquired nothing in the run removes them — they survive mutation, are inherited in full by every offspring, and are lost only when the line dies.

**How you could tell it works in-game**
After a mutation that rewrites every limb colour, the gill slits / hooked ridges / matted shell / glowing seam are still on the face and the player can still cross the matching terrain; every offspring in a litter shows all traits the parent held; on death all four are gone next run.

**Data it implies**
Run state: set of held traits, separate from limb slots

**Worked example from the document**
Breeding can never remove a trait a later biome requires, so the breed-and-lose-a-trait soft-lock cannot occur.

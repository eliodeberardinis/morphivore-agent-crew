...
```
# Requirement: Unity MCP editor bridge  (GDD 3.3)

**What the design requires**
A custom Unity Editor plugin (MCPBridge.cs / MCPBridgeWindow.cs) listens on a local socket only and exposes write_file, refresh_assets, play and get_components; agents write only to Assets/Scripts/ and docs/ (never binaries), and validation is asset refresh, then grepping ~/Library/Logs/Unity/Editor.log for 'error CS' as the source of truth, then Play mode. The bridge is unreachable during domain reload and callers must wait and re-ping.

**How you could tell it works in-game**
Issuing write_file then refresh_assets over the local socket updates the project; during domain reload pings fail for a bounded window and then succeed; compile errors are found in Editor.log rather than the bridge's own log.

**Data it implies**
(none)

**Worked example from the document**
The bridge is unreachable for 8–13 s during domain reload, so agents wait and re-ping.

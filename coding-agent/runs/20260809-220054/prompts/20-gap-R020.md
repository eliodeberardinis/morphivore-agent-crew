...
```
# Requirement: Next-slot-to-be-overwritten indicator  (GDD 2.4a)

**What the design requires**
The limb that the next meal will overwrite (the oldest colour) pulses on the player's body at all times.

**How you could tell it works in-game**
One limb visibly pulses; after eating or pooping the pulse moves to the new oldest colour, so a colour is never lost unannounced.

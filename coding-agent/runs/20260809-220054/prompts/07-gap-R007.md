...
```
# Requirement: Eating a downed creature  (GDD 2.1, 2.3)

**What the design requires**
Pouncing a creature that is already dizzy eats it, consuming the creature and applying its meat effect (colour+tier to the buffer for coloured prey, healing for grazers), with distinct visuals per meal type.

**How you could tell it works in-game**
A second pounce on a downed creature removes it from the world; prey meat visibly streams into one of the player's limbs, grazer meat dissolves into the body.

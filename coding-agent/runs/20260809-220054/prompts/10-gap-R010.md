...
```
# Requirement: Grazers as the healing supply  (GDD 2.3)

**What the design requires**
Grazers are harmless, no-colour, limbless white herbivores scattered thinly in every biome; they never fight, bolt the instant they are locked, and eating one restores a chunk of current-maximum Health without changing the player's form. Their flee speed is authored below every form's dash speed, and they never count toward an Alpha's eat-count.

**How you could tell it works in-game**
Locking a grazer makes it run immediately; the player can always catch it by dashing regardless of current form; eating it refills part of the Health bar while the body and limb colours stay unchanged, and the Alpha prey counter does not increase.

**Data it implies**
creatures.json: role=grazer entries with no colour family, no meat tier, grazer flee speed, home biomes

**Worked example from the document**
Grazer flee speed in creatures.json must be lower than Speed × family Dash multiplier for every one of the 150 forms.

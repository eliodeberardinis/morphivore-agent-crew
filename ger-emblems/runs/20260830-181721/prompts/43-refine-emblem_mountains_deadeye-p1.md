# Prompt 43 — refine-emblem_mountains_deadeye-p1
...
name: "Ledge-Watcher's Brow"
flavor: 'The bony shelf it sighted from, cracked off with the dust of its last perch still on it.'
id: 'emblem_mountains_deadeye'  alpha_id: 'alpha_mountains_blue'
biome: 'mountains'  opens_biome: 'beach'

# What review found

- [format.id_matches_alpha] field `id` (deterministic check): id is 'emblem_mountains_deadeye' but must be 'emblem_mountains_blue' — emblem_<biome>_<family>, using the Alpha's colour family (Blue), not its name.
```

...
```
# Requirement: Intensity scaling of family multipliers  (GDD 2.4, 2.4b)

**What the design requires**
An intensity tier scales each family multiplier's distance from 1.0: Pale 25%, Dusk 50%, Deep 75%, Clash ~90% plus a minor second-tone gift, Rage 100%. The Clash gift is authored per form.

**How you could tell it works in-game**
A Pale Yellow form plays only slightly tougher and slower than the blank, while a Rage Yellow form expresses the full 1.5/0.85/1.5 trade; a Clash form shows near-full expression plus its authored second-tone bonus.

**Data it implies**
forms.json: intensity field, stat profile derived from the scaling rule (hand-tuned), per-form Clash gift

**Worked example from the document**
Pale Yellow Health multiplier = 1 + 0.25×(1.5−1) = 1.125; Deep Yellow = 1.375; Rage Yellow = 1.5.

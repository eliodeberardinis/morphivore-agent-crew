...
```
# Requirement: Alpha lair and the two-gate unlock  (GDD 2.5)

**What the design requires**
Each biome contains one visible Alpha lair landmark that stays empty until both gates are met: a count of distinct forms taken in that biome and a count of coloured prey eaten there (Prairies 3/15, Wetlands 5/20, Mountains 7/25, Beach 9/30, Volcanic 11/35). At 90% of both gates the lair stirs.

**How you could tell it works in-game**
The lair is visible on arrival but inert; meeting only the eat-count or only the form count leaves it inert; at 90% of both it visibly stirs, and on clearing both the Alpha appears.

**Data it implies**
biomes.json: distinct-forms gate and prey-eaten gate per biome; run state: per-biome distinct forms and prey-eaten counters

**Worked example from the document**
Gate table: Prairies 3 distinct/15 prey (15 reachable forms, 5.0× slack), Wetlands 5/20 (20, 4.0×), Mountains 7/25 (25, 3.6×), Beach 9/30 (25, 2.8×), Volcanic 11/35 (25, 2.3×).

...
```
# Requirement: The Bestiary screen  (GDD 2.9)

**What the design requires**
A single Bestiary screen shows rows = limb count (1–6) and columns = family (5); each of the 30 cells opens into that family's intensity ladder (Pale → Dusk → Deep → Clash → Rage) for 150 forms total. Reached forms show in full colour with their authored name, unreached ones as dark silhouettes; the screen also lists every emblem ever taken. It opens from the pause and main menus, its header shows discovered / total and the next milestone, forms found this run are highlighted with Alpha-threshold progress, and offspring the player did not pick appear outlined as known but unachieved.

**How you could tell it works in-game**
Opening the Bestiary from either menu shows a 6×5 grid; clicking a cell reveals five intensity entries; a newly eaten form flips from silhouette to named colour art; declining an offspring adds an outlined entry; the header reads e.g. discovered/150 and the next unlock.

**Data it implies**
Persisted Bestiary: discovered form ids, emblems claimed, per-run highlight set

**Worked example from the document**
30 cells × 5 intensities = 150 forms displayed.

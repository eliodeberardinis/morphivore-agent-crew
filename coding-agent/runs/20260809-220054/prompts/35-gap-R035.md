...
```
# Requirement: Decorated panels  (GDD 2.8, 3.3)

**What the design requires**
Elite creatures only drop decorated meat panels that bolt onto the player's body, each granting a power (longer lock range, stronger dash, tougher guard, brief camouflage). Panels are the run's only power source; capacity grows with the body, panels attach anywhere across the cube's faces without crowding main-face traits, at capacity the player sheds one, the same power does not stack, and they survive mutation and breeding.

**How you could tell it works in-game**
Killing an elite drops a visible panel that studs onto the cube and measurably changes the named stat; a second copy of the same panel does not double the effect; a bigger beast carries more panels; panels are still attached after a mutation and after breeding.

**Data it implies**
panels.json: power, magnitude, which elites drop it, attachment point; run state: attached panels and capacity
